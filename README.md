...
## Запуск через docker
1) В командной строке вызвать "docker build --build-arg ttoken="put your own token" ." и дождаться, когда соберется образ
2) С помощью команды "docker images" найти новый образ и скопировать его "IMAGE ID"
3) Запустить команду "docker run IMAGE ID (полученный на прошлом шаге)" 
4) Дождаться пока образ поднимется
